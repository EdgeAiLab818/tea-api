...
【計算ドラフト】の情報を、簡潔に整えてください。

【計算ドラフト】:
{raw_answer}

【清書ルール】:
1. **挨拶の禁止**: 「ようこそ」「こんにちは」等の挨拶は不要です。即座に回答を開始してください。
2. **情報の完全維持**: ドラフトにある「全ての商品名」と「価格」を必ず含めてください。要約は禁止です。
3. **重複チェック**: 回答内で同じ商品が繰り返されていないか確認し、整理してください。
